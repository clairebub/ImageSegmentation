...
class UNetTwoDecoder(nn.Module):
    def __init__(self, num_classes, input_channels=3, deep_supervision=False, **kwargs):
        super().__init__()
        ...
        self.conv3_1_1 = VGGBlock(nb_filter[3] + nb_filter[4], nb_filter[3], nb_filter[3])
        self.conv2_2_1 = VGGBlock(nb_filter[2] + nb_filter[3], nb_filter[2], nb_filter[2])
        self.conv1_3_1 = VGGBlock(nb_filter[1] + nb_filter[2], nb_filter[1], nb_filter[1])
        self.conv0_4_1 = VGGBlock(nb_filter[0] + nb_filter[1], nb_filter[0], nb_filter[0])
        self.final_1 = nn.Conv2d(nb_filter[0], 1, kernel_size=1)

        self.conv3_1_2 = VGGBlock(nb_filter[3] + nb_filter[4], nb_filter[3], nb_filter[3])
        self.conv2_2_2 = VGGBlock(nb_filter[2] + nb_filter[3], nb_filter[2], nb_filter[2])
        self.conv1_3_2 = VGGBlock(nb_filter[1] + nb_filter[2], nb_filter[1], nb_filter[1])
        self.conv0_4_2 = VGGBlock(nb_filter[0] + nb_filter[1], nb_filter[0], nb_filter[0])
        self.final_2 = nn.Conv2d(nb_filter[0], 1, kernel_size=1)
    ...
    def forward(self, input):
        ...
        x3_1_1 = self.conv3_1_1(torch.cat([x3_0, self.up(x4_0)], 1))
        x2_2_1 = self.conv2_2_1(torch.cat([x2_0, self.up(x3_1_1)], 1))
        x1_3_1 = self.conv1_3_1(torch.cat([x1_0, self.up(x2_2_1)], 1))
        x0_4_1 = self.conv0_4_1(torch.cat([x0_0, self.up(x1_3_1)], 1))
        output_1 = self.final_1(x0_4_1)

        x3_1_2 = self.conv3_1_2(torch.cat([x3_0, self.up(x4_0)], 1))
        x2_2_2 = self.conv2_2_2(torch.cat([x2_0, self.up(x3_1_2)], 1))
        x1_3_2 = self.conv1_3_2(torch.cat([x1_0, self.up(x2_2_2)], 1))
        x0_4_2 = self.conv0_4_2(torch.cat([x0_0, self.up(x1_3_2)], 1))
...
# num_classes, input_channels=3, **kwargs
...
class R2U_Net(nn.Module):
    def __init__(self, num_classes, input_channels=3, t=2, **kwargs):
        ...
        self.RRCNN1 = RRCNN_block(ch_in=input_channels, ch_out=nb_filter[0], t=t)
...
class AttU_Net(nn.Module):
    def __init__(self, num_classes, input_channels=3, **kwargs):
        ...
        self.Up2 = up_conv(ch_in=nb_filter[1], ch_out=nb_filter[0])
        self.Att2 = Attention_block(F_g=nb_filter[0], F_l=nb_filter[0], F_int=32)
        self.Up_conv2 = conv_block(ch_in=nb_filter[1], ch_out=nb_filter[0])
